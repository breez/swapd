...
def apply_tweak(
    keyagg_ctx: KeyAggContext, tweak: bytes, is_xonly: bool
) -> KeyAggContext:
    if len(tweak) != 32:
...
def key_agg_and_tweak(
    pubkeys: List[PlainPk], tweaks: List[bytes], is_xonly: List[bool]
) -> KeyAggContext:
...
def get_session_values(
...
) -> Tuple[Point, int, int, int, Point, int]:
    ...
    e = (
        int_from_bytes(tagged_hash("BIP0340/challenge", xbytes(R) + xbytes(Q) + msg))
        % n
    )
...
def det_nonce_hash(
    sk_: bytes, aggothernonce: bytes, aggpk: bytes, msg: bytes, i: int
) -> int:
    buf = b""
    buf += sk_
    buf += aggothernonce
...
def partial_sig_verify(
    ...
    if len(pubnonces) != len(pubkeys):
        raise ValueError(
            "The `pubnonces` and `pubkeys` arrays must have the same length."
        )
    if len(tweaks) != len(is_xonly):
        raise ValueError(
            "The `tweaks` and `is_xonly` arrays must have the same length."
...
def partial_sig_verify_internal(
    psig: bytes, pubnonce: bytes, pk: bytes, session_ctx: SessionContext
) -> bool:
    (Q, gacc, _, b, R, e) = get_session_values(session_ctx)
...
def test_key_agg_vectors() -> None:
    ...
    for test_case in error_test_cases:
        ...
        assert_raises(
            exception, lambda: key_agg_and_tweak(pubkeys, tweaks, is_xonly), except_fn
        )
...
def test_nonce_gen_vectors() -> None:
    ...
    for test_case in test_data["test_cases"]:
        ...
        assert nonce_gen_internal(rand_, sk, pk, aggpk, msg, extra_in) == (
            expected_secnonce,
            expected_pubnonce,
        )
...
def test_sign_verify_vectors() -> None:
    ...
    for test_case in valid_test_cases:
        ...
        assert partial_sig_verify(
            expected, pubnonces, pubkeys, [], [], msg, signer_index
        )
    ...
    for test_case in verify_fail_test_cases:
        ...
        assert not partial_sig_verify(
            sig, pubnonces, pubkeys, [], [], msg, signer_index
        )
...
def test_tweak_vectors() -> None:
    ...
    for test_case in valid_test_cases:
        ...
        assert sign(secnonce_tmp, sk, session_ctx) == expected
        assert partial_sig_verify(
            expected, pubnonces, pubkeys, tweaks, is_xonly, msg, signer_index
        )
...
def test_det_sign_vectors() -> None:
    ...
    for test_case in valid_test_cases:
        ...
        assert partial_sig_verify_internal(
            psig, pubnonce, pubkeys[signer_index], session_ctx
        )
    ...
    for test_case in error_test_cases:
        ...
        try_fn = lambda: deterministic_sign(
            sk, aggothernonce, pubkeys, tweaks, is_xonly, msg, rand
        )
    ...
    for i in range(iters):
        ...
        if i % 2 == 0:
            ...
            secnonce_2, pubnonce_2 = nonce_gen(
                sk_2, pk_2, aggpk, msg, t.to_bytes(8, "big")
            )
        else:
            aggothernonce = nonce_agg([pubnonce_1])
            rand = secrets.token_bytes(32)
            pubnonce_2, psig_2 = deterministic_sign(
                sk_2, aggothernonce, pubkeys, tweaks, is_xonly, msg, rand
            )
        ...
        assert_raises(
            ValueError, lambda: sign(secnonce_1, sk_1, session_ctx), lambda e: True
        )

        # Wrong signer index
        assert not partial_sig_verify(
            psig_1, pubnonces, pubkeys, tweaks, is_xonly, msg, 1
        )

        # Wrong message
        assert not partial_sig_verify(
            psig_1, pubnonces, pubkeys, tweaks, is_xonly, secrets.token_bytes(32), 0
        )
